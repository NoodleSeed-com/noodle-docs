...
const EvolutionLogPage = () => {
  const [activeSection, setActiveSection] = useState<number>(1);
  const entryRefs = useRef<(HTMLDivElement | null)[]>([]);
  
  // Generate navigation items for the left sidebar
  const navItems = [
    { id: 'evolution-log', label: 'Evolution Log', href: '/evolution-log' },
  ];
  ...
  return (
    <div className="doc-container">
      <h1 className="page-title">🍜 Evolution Log: Noodle's Journey</h1>
          ...
          <ul className="changelog-nav-list">
            {navItems.map((item) => (
              <li key={item.id} className="changelog-nav-item">
                <Link 
                  to={item.href} 
                  className={`changelog-nav-link ${item.id === 'evolution-log' ? 'active' : ''}`}
                >
                  {item.label}
                </Link>
              </li>
...
export default EvolutionLogPage;
